...
## σ bond formation — phases (not legacy “Step 1/2/3”)

Use this vocabulary for **σ bonds only**. **Guide** / **non-guide** resolution and orchestrator calls are **setup** (not an animation phase).

- **Prebonding + formation (one clock):** translate the **non-guide** fragment and run **non-guide** electron-domain motion on **one** timeline: `smoothstep(elapsed / bondAnimStep1Duration)` in `ElectronOrbitalFunction`, with wall time before `Create` = `bondAnimStep1Duration * sigmaFormationPrebondWallFraction` and the remainder after `Create` for forming σ, bond-line rows, joint, σ-relax (same master curve; not `elapsed/(step1+step2)` as two inspector totals).
- **Bonding event**: create the `CovalentBond`, place the σ line visual, then transition to the cylinder / line view — *implementation*: `Create` plus `bondAnimStep2Duration` for `AnimateOrbitalToLine` (fallback `bondAnimStep3Duration` if step 2 is 0).
- **Postbonding**: **guide**-atom orbital redistribution / hybrid alignment when the pipeline needs it (conditional; not a separate duration field on `ElectronOrbitalFunction`).

**Electron overlap avoidance** during formation: ensure two electrons don’t overlap—rotate one orbital 180° when needed (see `orbitalRotationFlipped` / `CovalentBond`).

**π bonds** and other cases: use the π-specific sections below; do not assume the old three-step σ list above (removed).

**Full σ sequencing (setup, prebonding, bonding event, postbonding)** is documented in `instruction/electron-redistribution-orbital-drag-events.md`.
...
| `ElectronOrbitalFunction` | Orbital; `electronCount`, `electronPrefab`, `stretchSprite` (optional). `Bond` property when part of covalent bond. Spawns `ElectronFunction` children via `SyncElectronObjects()`. **σ bond formation** follows **prebonding → bonding event → postbonding** (see `instruction/electron-redistribution-orbital-drag-events.md`); `bondAnimStep1Duration` + `sigmaFormationPrebondWallFraction` = one combined formation clock; `bondAnimStep2Duration` = orbital-to-line (legacy `bondAnimStep3Duration` fallback). Sigma bonds use flip logic and `RearrangeOrbitalToFaceTarget` (with `OrbitalAngleUtility`); pi bonds (`alreadyBonded`) skip rearrange/align and just form the bond. Flip uses `IsSourceOrbitalAlreadyAlignedWithTarget`, `IsOrbitalAlignedWithTargetPoint`, and `sourceOrbitalOriginalTip` for target alignment. `RearrangeOrbitalToFaceTarget` accepts optional `targetPointOverride` for flip case. Sigma and pi bond formation call `GetRedistributeTargets(piBefore, bondPartner)` where applicable. `SwapPositionsWith` for same-atom orbital swap. |
